...
[![Typing SVG](https://readme-typing-svg.demolab.com?font=Times+New+Roman&center=true&weight=600&pause=1000&color=6CAAF7&width=435&lines=Human-Centric+Intelligent+Software+Development)](https://git.io/typing-svg)
...
- 🌍 **ACM Conference on Human Factors in Computing Systems** 2025 Student Design Competition **Finalist** (Top 4)

### 🔮 [**크로스가우시안(CrossGaussian)**](https://github.com/BynPlay/CrossGaussian): 비디오 학습 기반 뉴럴 렌더링과 룸스케일 공간 공유 기반 원격 협업 인터랙션 연구
- **역할**: 연구 리드 & 기술 설계 및 통합 [1저자] | **팀 구성**: 게임 엔진 2 · 백엔드 2 · 데이터 분석 1
- **소개:** 원격 헙업에서의 정보 비대칭을 해소하기 위해 3D 뉴럴 렌더링 + 360° 영상 스트리밍을 접목해, 룸스케일 단위 협업 공간 공유와 상호작용 가능성을 탐구하는 연구 프로젝트. **[설계]** 선행 연구 리뷰 및 연구 공백 확인, 3DGS 기반 공간 탐색 및 시각적 상호작용을 위한 디자인 스페이스 설계 **[백엔드]** Unity·HLSL 기반 클라이언트와 Python·CUDA 파이프라인을 연동해 360° 스트리밍·영상 데이터 수집·원격 3DGS 재구성 자동화 **[클라이언트]** 렌더링 정보 기반 인터랙션 기법 구현 및 N=24 사용자 실험을 통한 가설 검증 통계 분석 실행
...
### 🚃 [**트램스케이프 투 코리아(TramscapeToKoea)**](https://github.com/BynPlay/TramScape-To-Korea): 카메라 센서 움직임으로 상호작용하는 실감형 기차 여행
- **역할**: 인터랙션 설계 & 센서 시스템 개발 | **팀 구성**: 아트 1 · 프로그래밍 4 
- **소개:** 플레이어의 신체 움직임에 따라 창문을 들여다보듯 시점이 변화하며, 나레이션과 함께 한국의 주요 관광 명소를 탐험하는 열차 관광 게임 **[설계]** 2D 화면에서도 입체감을 제공하는 "창문" 인터랙션 메타포 설계 및 구현 (인지심리이론 기반) **[백엔드]** Azure Kinect SDK를 이용해 센서 데이터 수집, 디지털 필터링을 통해 노이즈 제거. 필터링된 데이터를 바탕으로 사용자의 눈 위치를 추적하고, 해상도와 크기 정보에 따라 가상 카메라 절두체(projection frustum)를 실시간으로 왜곡하여 디스플레이에 입체감 효과 구현. **[클라이언트]** 한국 랜드마크 모델링 및 HDRP 포스트 프로세싱으로 환경 구현, 스플라인 기반 열차 이동 시스템과 AI 투어 음성 등 콘텐츠 시나리오 구현
...
### 🌟 [**비벤(Viven)**](http://journal.dcs.or.kr/_common/do.php?a=full&b=12&bidx=3703&aidx=40968): 경희대학교 온라인 교육용 토의·토론 특화 메타버스 플랫폼
- **역할**: 서비스 기획 & 프로토타입 개발 | **팀 구성**: 기획 1 · 기획 & 프로그래밍 1
...
- 🎮 [**DirectX 11 Graphics Renderer**](https://github.com/BynPlay/GameGraphics-Engine-DirectX) — DirectX 11 + WINAPI 기반 커스텀 3D 렌더링 엔진 구현 실습
- 🌄 [**Panoramic Image Stitching**](https://github.com/BynPlay/Panoramic-Stitching) — Harris → Matching → RANSAC → Homography 파이프라인 Python 알고리즘 구현 실습
- 🦿 [**Unreal Engine Kinematics**](https://github.com/DevByn/Kinematics-Unreal) 언리얼 엔진 블루프린트 기반 FK/IK 로보틱스 시뮬레이션 구현 실습
- 📉 [**Handtracking LSTM Optimization**](https://github.com/DevByn/LSTM-HandTracking) IMU 데이터셋, LSTM 모델 기반 핸드트레킹 예측 최적화 실습
- ⚡ [**Object-oriented 4D Convolution Engine**](https://github.com/DevByn/CNN-Implementation) 이미지 해상도 향상을 위한 4차원 컨볼루션 연산 객체지향 방식 구현 실습
- ✋ [**Microgravity Hand Simulation**](https://github.com/BynPlay/Microgravity-Hand-Simualtion) — Leap Motion 기반 손 추적 미소중력 시뮬레이션 및 충돌 지점 시각화 프로젝트
- 👁️ [**Handtracking LSTM Optimization**](https://github.com/DevByn/LSTM-HandTracking) 메타 퀘스트 프로 Lay Casting 기반 아이트레킹 오브젝트 상호작용 알고리즘 구현 프로젝트
---
